...
Auto Wallet Backup
---------------------
In addition to the automatic wallet backup that is done at each start of the client, a new automatic backup function has been added that will, by default, create a backup of the wallet file during each zBABY mint operation (zBABY spends which re-mint their change are also included in this). This functionality is controlled by the `-backupzbaby` command-line option, which defaults to `1` (enabled, auto-backup).

Users that wish to prevent this behavior (not recommended) can pass `-backupzbaby=0` at the command-line when starting the client, or add `backupzbaby=0` to their `baby.conf` file.
...
- #279 `e734010` Add -backupzbaby startup flag. (presstab)
