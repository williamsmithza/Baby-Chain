...
CzBABYWallet::CzBABYWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbaby seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBABYSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBABYSeed_deprecated()) {
                    LogPrintf("%s: Updated zBABY seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbaby seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zbaby wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzBABYWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBABYCount(nCountLastUsed);
    else if (!walletdb.ReadZBABYCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBABYWallet::Lock()
{
    seedMaster = 0;
}

void CzBABYWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBABYWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBABY(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBABYWallet::LoadMintPoolFromDB()
{
...
void CzBABYWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBABYWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBABYWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbabyTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBABYWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBABY(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbabyTracker which also adds to database
    pwalletMain->zbabyTracker->Add(dMint, true);
...
void CzBABYWallet::SeedToZBABY(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBABYWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBABYWallet::GenerateDeterministicZBABY(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBABYWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBABYWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
