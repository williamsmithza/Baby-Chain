...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZBabyStake(spend));
    } else {
        ...
        CBabyStake* babyInput = new CBabyStake();
        babyInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(babyInput);
    }
