...
CzBABYTracker::CzBABYTracker(std::string strWalletFile)
{
...
CzBABYTracker::~CzBABYTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzBABYTracker::Init()
{
...
bool CzBABYTracker::Archive(CMintMeta& meta)
{
...
bool CzBABYTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzBABYTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzBABYTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzBABYTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzBABYTracker::GetSerialHashes()
{
...
CAmount CzBABYTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzBABYTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzBABYTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzBABYTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzBABYTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzBABYTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzBABYTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzBABYTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzBABYTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzBABYTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzBABYTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBABYTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzBABYTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzBABYTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzBABYTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzBABYTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzBABYTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzbaby from DB\n", __func__, listDeterministicDB.size());
    }
