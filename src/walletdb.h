...
// Copyright (c) 2016-2018 The BABY developers
...
public:
    ...
    bool WriteZBABYSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZBABYSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZBABYSeed_deprecated(uint256& seed);
    bool EraseZBABYSeed();
    bool EraseZBABYSeed_deprecated();

    bool WriteZBABYCount(const uint32_t& nCount);
    bool ReadZBABYCount(uint32_t& nCount);
