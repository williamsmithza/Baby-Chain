...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-babystake=<n>", strprintf(_("Enable or disable staking functionality for BABY inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbabystake=<n>", strprintf(_("Enable or disable staking functionality for zBABY inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzbaby=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBABY minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zbabybackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zBABY backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The BABY Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //BABY: Load Accumulator Checkpoints according to network (main/test/regtest)
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        zwalletMain = new CzBABYWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zBABYWallet
        uiInterface.InitMessage(_("Syncing zBABY wallet..."));

        bool fEnableZBabyBackups = GetBoolArg("-backupzbaby", true);
        pwalletMain->setZBabyAutoBackups(fEnableZBabyBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zbabyTracker->Init();
