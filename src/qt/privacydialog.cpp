...
void PrivacyDialog::sendzBABY()
{
    ...
    if (!ZBabyControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZBabyControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBABY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zBABY"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZBABY_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zBABY require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zBABY"));
            ui->TEMintStatus->repaint();
            return;
    ...
    // Clear zbaby selector in case it was used
    ZBabyControlDialog::setSelectedMints.clear();
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zbabyTracker->GetMints(false);
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zBABY "));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zBABY </b> "));
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzBABY->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        ...
        // Mint zBABY
        ui->pushButtonMintzBABY->setEnabled(true);
        ui->pushButtonMintzBABY->setToolTip(tr("PrivacyDialog", "Enter an amount of BABY to convert to zBABY", 0));

        // Spend zBABY
        ui->pushButtonSpendzBABY->setEnabled(true);
        ui->pushButtonSpendzBABY->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
